/**
 * React functional component that displays a user profile page.
 * Fetches user profile data from Redux store and renders profile information.
 * Provides options to follow/unfollow user, update profile, and send a message.
 *
 * @returns {JSX.Element} Rendered user profile page
 */
import {
  EmojiSadIcon,
  HeartIcon,
...
export default function Profile() {
  ...
  const sendMailNavigate = () => {
    navigate('/send-email', {
      state: { email: profile?.email, id: profile?.id }
    });
  };
                          ...
                          <div className='mt-6 sm:flex-1 sm:min-w-0 sm:flex sm:items-center sm:justify-end sm:space-x-6 sm:pb-1'>
                            <div className=' flex flex-col 2xl:block mt-10 min-w-0 flex-1'>
                              ...
                              <p className='text-green-600 mt-2 mb-2'>
                                ...
                                {profile?.following?.length} followed
                              </p>
                              {/* Who view my profile */}
                                ...
                                <div>
                                  {profile?.isFollowing ? (
                                    <button
                                      onClick={() =>
                                        dispatch(
                                          unfollowUserAction(profile?._id)
                                        )
                                      }
                    ...
                    <div className='flex justify-center place-items-start flex-wrap  md:mb-0'>
                      <div className='w-full md:w-1/3 px-4 mb-4 md:mb-0'>
                        <h1 className='text-center text-xl border-gray-500 mb-2 border-b-2'>
                          Profile Views [{profile?.viewedBy?.length}]
                        </h1>
                        {/* Who view my post */}
                        <ul className=''>
