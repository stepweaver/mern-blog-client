/**
 * This code snippet is a functional component in React that represents a login form. It uses the `useFormik` hook for form handling and validation, and the `useNavigate` hook from React Router for navigation. It also utilizes the `useSelector` and `useDispatch` hooks from React Redux for managing state and dispatching actions. The form has fields for email and password, with validation using the Yup library. Upon submission, the form dispatches a login action and redirects to the user's profile page if authentication is successful.
 *
 * Example Usage:
 *
...
const Login = () => {
  ...
  if (userAuth) {
    return navigate(`/profile/${userAuth?._id}`);
  }

  return (
    <>
      <section className='min-h-screen relative py-20 2xl:py-40 bg-gray-900 overflow-hidden'>
        <div className='absolute top-0 left-0 lg:bottom-0 h-full lg:h-auto w-full lg:w-4/12 bg-gradient-to-r from-gray-900 to-blue-900 lg:overflow-hidden'>
          <img
          ...
          <div className='max-w-5xl mx-auto'>
            <div className='flex flex-wrap items-center -mx-4'>
              <div className='w-full lg:w-2/5 px-4'>
                <div className='px-6 lg:px-12 py-12 lg:py-24 backdrop-blur-lg shadow-lg rounded-lg'>
                  {/* Form */}
                  <form onSubmit={formik.handleSubmit}>
                    <h3 className='mb-10 text-2xl font-bold font-heading text-gray-50 text-center'>
                      {/* Header */}
                      Login to your account
                      {/* display error message */}
                      {appErr || serverErr ? (
                        <div className='text-red-600 mt-1 text-center'>
                          {serverErr} - {appErr}
                        </div>
                      ) : null}
                    </h3>
                    ...
                    {loading ? (
                      ...
                      <button
                        type='submit'
                        className='py-4 w-full bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-full transition duration-200'
                      >
                        Login
                      </button>
                    )}
                  </form>
                  <div className='py-2 w-full text-center font-medium text-blue-50 hover:text-blue-500'>
                    <Link to='/reset-password-token'>Forget password?</Link>
                  </div>
                </div>
              </div>
              <div className='w-full lg:w-3/5 px-4 mb-16 lg:mb-0 order-first lg:order-last'>
                <span className='flex mb-10 mx-auto items-center justify-center h-20 w-20 bg-blue-600 rounded-lg'>
                  <svg
                    width='37'
                    height='37'
                    viewBox='0 0 37 37'
